...
func (ac *AdminController) GetSystemStats(c *gin.Context) {
	...
	// Share statistics
	database.GetDB().Model(&models.ShareLink{}).Count(&stats.TotalShares)
	database.GetDB().Model(&models.ShareLink{}).Where("is_active = ?", true).Count(&stats.ActiveShares)
	database.GetDB().Model(&models.ShareLink{}).Where("expires_at < ?", "NOW()").Count(&stats.ExpiredShares)
